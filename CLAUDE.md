...
cargo test               # unit tests live in each module's #[cfg(test)] block; cargo test runs all (currently 88)
...
Nine source files plus build-time helpers:
...
- [src/confusables.rs](src/confusables.rs) — runtime confusable model: `ConfusableMap` (active single-char map + digraph list built from the selected sources), `parse_sources` (validates the `--confusables` comma-list and returns `Vec<Source>`), and `ConfusableMap` methods `skeleton_of`, `skeleton`, and `confusable` (formerly in `distance.rs`). The map is source-parameterized and threaded through `PairContext`.
...
- [src/flowcrypt_data.rs](src/flowcrypt_data.rs) — **auto-generated, do not hand-edit.** FlowCrypt `idn-homographs-database` single-char supplement: a `pub static FLOWCRYPT: &[(u32, u32)]` slice + provenance constants (`FLOWCRYPT_SOURCE_COMMIT`, `FLOWCRYPT_SOURCE_DATE`). Generated by `scripts/gen_flowcrypt.py`; the 19 MB source JSON is not committed.
