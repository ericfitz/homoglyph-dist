...
- [src/verdict.rs](src/verdict.rs) — `Verdict` enum + `verdict()` (single-pair human spoof/benign tags), plus `TyposquatClass` + `classify_typosquat()` for the `--typosquat` four-way classification (`identical` / `same_project` / `likely_typosquat` / `unrelated`). Classification is **not** an axis — it is never in `ALL_AXES` / `--fields`.
...
**`--typosquat` profile:** opt-in package-registry detection profile. Default emit set is five axes (`equal`, `damerau`, `skeleton_damerau`, `confusable_only`, `keyboard_distance`); default `--metric` becomes `damerau` (unless `-m` was passed). Classifies each pair as `identical` / `same_project` / `likely_typosquat` / `unrelated` and emits `classification` + `reason` JSON keys (after axes). In batch/list mode, emits **only** `likely_typosquat` rows and exits 1 if none. Cannot combine with `-t`/`--threshold`. `classification` is not an axis — `--fields` remains axis-only.
...
**Single-pair human verdict:** Single-pair human output appends an interpretation verdict line. Default mode: `[IDENTICAL]`, `[LIKELY SPOOF]`, or `[LIKELY BENIGN]` from `verdict()` in `verdict.rs` (NOT in JSON or batch modes). Spoof rule: `confusable_only || (homoglyph_share > 0.5 && max(len) >= 3 && len_diff_ratio <= len_tolerance)`, where `homoglyph_share = uts39_skeleton_delta / damerau`. Tune with `--len-tolerance` (default 0.25, range [0,1]). With `--pypi`/`-n` and no `--typosquat`, identity-gated pairs get `[SAME PROJECT]` instead. With `--typosquat`, tags are `[IDENTICAL]` / `[SAME PROJECT]` / `[LIKELY TYPOSQUAT]` / `[UNRELATED]` from `classify_typosquat()`.

**Batch exit codes:** Batch modes (`--stdin`, `--list`) with `-t` exit 1 when zero rows match; 0 otherwise. Without `-t`, always exit 0. With `--typosquat`, batch/list emit only `likely_typosquat` rows and exit 1 if none (cannot combine with `-t`).
...
**Batch and list output:** always JSONL (one JSON object per line). With `-t` threshold: emits only lines at/under the threshold. `-m/--metric` (default `skeleton_damerau`) selects which numeric axis drives `-t` and `--sort`. With `--typosquat`: emits only `likely_typosquat` rows.
